...
const controlActions = {
  ...
  'speech.say': { description: 'Queue one line for local emotion classification, PocketTTS, and LAM processing.', args: { text: 'string', expression: 'auto|none|declared VRM expression optional', curve: 'array of normalized 0..1 weights or {time,value} points optional' } },
  'speech.queue.clear': { description: 'Remove speech that is not currently processing or playing.', args: {} },
  'speech.schedule.set': { description: 'Replace scheduled speech, including per-line automatic, disabled, or explicit expression envelopes.', args: { cues: [{ time: 'seconds >= 0', text: 'string', expression: 'auto|none|declared VRM expression optional', curve: 'normalized envelope optional' }], loop: 'boolean optional' } },
  ...
  'embedding.idle-pair.set': { description: 'Set primary and secondary idle embeddings that alternate only while no expression embedding is active.', args: { primary: 'cache key, nickname, or exact text', secondary: 'cache key, nickname, or exact text optional', intervalSeconds: '1..300' } },
  ...
  'path.schedule.set': { description: 'Replace timed X/Z walk endpoints.', args: { endpoints: [{ time: 'arrival seconds >= 0', x: 'metres', z: 'metres' }], loop: 'boolean optional', curved: 'boolean optional', curveStrength: '0.1..1 optional' } },
  ...
  'export.single-pass': { description: 'Restart Live Full Flow and export one non-repeating pass of its speech, facial animation, motion, and camera as a compatible MP4.', args: {} },
  'motion.settings.set': { description: 'Set the Live Full Flow Core-40 generation controls. These values are independent from the ARDY tab.', args: { speed: '0.1..2.5 m/s optional', steeringBlend: '0.1..2 seconds optional', denoisingSteps: '1..10 optional', constraintGuidance: '0.5..4 optional', textGuidance: '0.5..5 optional', historyFrames: '4..80 in multiples of 4 optional', seamBlendFrames: '0..12 visual horizon handoff frames optional', adaptiveReplanBuffer: 'boolean optional', replanBufferFrames: '0..12 optional; used when adaptiveReplanBuffer is false', headingEnabled: 'boolean optional' } },
  'avatar.expression.set': { description: 'Set or clear one declared VRM expression layer without disabling LAM mouth, blink, or gaze animation.', args: { name: 'expression name or displayed label from control state', weight: '0..1; zero clears this expression' } },
  'avatar.expressions.clear': { description: 'Clear every manual VRM expression layer while leaving LAM facial animation active.', args: {} },
  'avatar.expression.schedule.set': { description: 'Replace independent timed VRM expression envelopes.', args: { cues: [{ name: 'declared VRM expression', start: 'seconds >= 0', end: 'seconds greater than start', curve: 'array of normalized 0..1 weights or {time,value} points' }] } },
  'camera.set': { description: 'Set live camera behavior or framing using the current transition style unless overridden.', args: { target: 'face|torso|hips|full optional', directionAnchor: 'world|face|torso|feet optional', follow: 'boolean optional', orbit: 'boolean optional', distance: '0.45..12 optional', yaw: '-180..180 degree offset from the selected direction anchor optional', pitch: '10..170 degrees optional', orbitSpeed: '-90..90 degrees/sec optional', smoothing: '0.5..20 optional', targetOffsetY: '-1..1 metres optional', transition: 'cut|move optional', transitionSeconds: '0.1..30 optional' } },
  'camera.cut': { description: 'Cut immediately to a new camera configuration.', args: { target: 'face|torso|hips|full optional', directionAnchor: 'world|face|torso|feet optional', distance: '0.45..12 optional', yaw: '-180..180 optional', pitch: '10..170 optional' } },
  'camera.move': { description: 'Smoothly revolve and ease to a new camera configuration. For a transition motivated by scheduled speech, a useful initial offset is speech cue time + about 1 second; callers remain free to choose any timing.', args: { target: 'face|torso|hips|full optional', directionAnchor: 'world|face|torso|feet optional', distance: '0.45..12 optional', yaw: '-180..180 optional', pitch: '10..170 optional', transitionSeconds: '0.1..30 optional' } },
  'camera.preset': { description: 'Apply a standard camera shot as a cut or smooth move.', args: { preset: 'face|torso|full', transition: 'cut|move optional', transitionSeconds: '0.1..30 optional' } },
  'camera.nudge': { description: 'Adjust the current camera without replacing unrelated settings.', args: { yaw: 'degree delta optional', pitch: 'degree delta optional', distance: 'metre delta optional', transition: 'cut|move optional', transitionSeconds: '0.1..30 optional' } },
  'camera.reset': { description: 'Restore the default torso-follow camera as a cut or smooth move.', args: { transition: 'cut|move optional', transitionSeconds: '0.1..30 optional' } },
};
