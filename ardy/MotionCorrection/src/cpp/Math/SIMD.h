...
// Translate SSE intrinsics to NEON. sse2neon defines its own FORCE_INLINE,
// so save and restore ours around the include.
#pragma push_macro("FORCE_INLINE")
#undef FORCE_INLINE
#include <sse2neon.h>
#pragma pop_macro("FORCE_INLINE")

// _mm_permutevar_ps is AVX, which sse2neon (SSE4.2 and below) does not
// provide. Emulate the variable 4-lane shuffle with a byte-table lookup:
// each 32-bit control lane selects a source lane via its low 2 bits.
inline __m128 _mm_permutevar_ps( __m128 a, __m128i control )
{
    uint32x4_t lane = vandq_u32( vreinterpretq_u32_m128i( control ), vdupq_n_u32( 3 ) );
    uint32x4_t byteIdx = vaddq_u32( vmulq_n_u32( lane, 0x04040404u ), vdupq_n_u32( 0x03020100u ) );
    return vreinterpretq_m128_u8( vqtbl1q_u8( vreinterpretq_u8_m128( a ), vreinterpretq_u8_u32( byteIdx ) ) );
}
#else
#error "Unsupported architecture: requires x86 (SSE) or ARM64 (NEON via sse2neon)"
#endif
