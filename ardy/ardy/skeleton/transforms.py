...
def global_rots_to_local_rots(global_joint_rots: torch.Tensor, skeleton):
    ...
    # skeleton.joint_parents is a registered buffer that follows the skeleton's
    # device (e.g. CUDA for a loaded model), while the input may be on CPU (e.g.
    # motion parsed on CPU for caching); align the index/eye to the data device.
    device = global_joint_mats.device
    parent_rot_mats = global_joint_mats[:, skeleton.joint_parents.to(device)]

    parent_rot_mats[:, skeleton.root_idx] = torch.eye(3, device=device, dtype=parent_rot_mats.dtype)  # the root joint
