...
from ..tools import compute_vel_angle, compute_vel_xyz
...
class MotionRepBase:
    ...
    def __init__(
        ...
        self.global_root_dim = self.motion_root_dim = self.root_slice.stop
        self.local_root_dim = sum(val.numel() for val in self.local_root_size_dict.values())

        self.local_motion_rep_dim = self.local_root_dim + self.body_dim
        self.global_motion_rep_dim = self.motion_rep_dim

        # Stats ship as a single mean.npy / std.npy pair covering the full
        # feature vector; the per-part stats below are slices of it.
        if stats_path:
            all_stats = Stats(stats_path)
            self.local_root_stats = all_stats.sliced(
                indices=(self.global_root_dim + np.arange(self.local_root_dim)).tolist(),
            )
            self.body_stats = all_stats.sliced(
                indices=(self.global_root_dim + self.local_root_dim + np.arange(self.body_dim)).tolist(),
            )
            self.global_root_stats = all_stats.sliced(
                indices=(0 + np.arange(self.global_root_dim)).tolist(),
            )
            self.stats = all_stats.sliced(
                indices=(0 + np.arange(self.global_root_dim)).tolist()
                + (self.global_root_dim + self.local_root_dim + np.arange(self.body_dim)).tolist(),
            )
    ...
    def randomize_first_heading(
        ...
        target_heading_angle = torch.rand(features.shape[0], device=features.device) * 2 * np.pi
    ...
    def extract_root(self, features: torch.Tensor) -> torch.Tensor:
        """Extract normalized/global root features from a full motion tensor."""
        return features[..., self.root_slice]

    def extract_body(self, features: torch.Tensor) -> torch.Tensor:
        """Extract body features from a full motion tensor."""
        return features[..., self.body_slice]

    def concat_root_body(self, root: torch.Tensor, body: torch.Tensor) -> torch.Tensor:
        """Concatenate root and body features into a full Ardy feature tensor."""
