...
from .skeleton import G1Skeleton34, SkeletonBase, fk
...
def extract_input_motion_from_constraints(
    ...
    # This gives fullbody constraints the last say on hip translations.
    ...
    for constraint in sorted_constraints:
        ...
        # Filter any constraint frames that exceed num_frames.
        ...
        # Handle Root2DConstraintSet separately - only assign root_2d at xz dimensions
        if isinstance(constraint, Root2DConstraintSet):
            root_2d = getattr(constraint, "root_2d", None)  # (K, 2)
            if isinstance(frame_indices, torch.Tensor):
                root_2d = root_2d[valid_mask]
            else:
                root_2d = root_2d[valid_positions]
            root_2d = _match_hip_dtype(root_2d)  # (K, 2) where K = len(frame_indices)
            hip_translations_input[frame_indices, 0] = root_2d[:, 0]  # x
            hip_translations_input[frame_indices, 2] = root_2d[:, 1]  # z
            continue
            ...
            if isinstance(frame_indices, torch.Tensor):
                ...
                root_2d = constraint.root_2d[valid_mask]
            else:
                global_rots = global_rots[valid_positions]
                global_positions = global_positions[valid_positions]
                root_2d = constraint.root_2d[valid_positions]

            root_positions = global_positions[:, skeleton.root_idx]  # (K, 3)
            # For EE constraints that do not include Hips, overwrite xz with root_2d.
            # If Hips is constrained, we want to keep its exact root trajectory.
            if isinstance(constraint, EndEffectorConstraintSet) and "Hips" not in getattr(
                constraint, "joint_names", []
            ):
                root_positions[:, 0] = root_2d[:, 0]  # x
                root_positions[:, 2] = root_2d[:, 1]  # z
...
def create_working_rig_from_skeleton(
    ...
    if isinstance(skeleton, G1Skeleton34):
        retarget_map = {
...
def post_process_motion(
    ...
    # Create working rig
    working_rig = create_working_rig_from_skeleton(skeleton)
    ...
    for b in range(batch_size):
        motion_postprocess.correct_motion(
            ...
            constraint_masks_dict_lst[b] if batched_constraints else constraint_masks_dict,
            contact_threshold,
            root_margin,
            working_rig,
